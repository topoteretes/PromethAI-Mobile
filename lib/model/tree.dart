...
extension Ext on Tree {
  ...
  List<String> getLeafPreferences(List<String> path) {
    var lastTree = getLeaf(path);
    var result = lastTree.preference;
    for (var option in lastTree.options) {
      if (result.contains(option.category) && option.preference.isNotEmpty) {
        result = [...result.where((e) => e != option.category), ...option.preference];
      }
    }
    return result;
  }
